...
APP = "Playlist → MP3 (Android, No FFmpeg)"
FONT_L, FONT_E = ("Roboto", 16), ("Roboto", 13)
...
if HAS_TK and __name__ == "__main__":
    # GUI --------------------------------------------------------------------
    root = tk.Tk(); root.title(APP); root.geometry("560x820")
    wrap = ttk.Frame(root, padding=16); wrap.pack(fill="both", expand=True)

    ttk.Label(wrap, text="Service:", font=FONT_L).pack(anchor="w")
    service_var = tk.StringVar(value="Spotify")
    svc = ttk.Frame(wrap); svc.pack(anchor="w", pady=4)
    ...
    url_var = tk.StringVar(); ttk.Entry(wrap, textvariable=url_var, font=FONT_E).pack(fill="x", pady=6)

    ttk.Label(wrap, text="Output Folder:", font=FONT_L).pack(anchor="w", pady=(10,0))
    row = ttk.Frame(wrap); row.pack(fill="x", pady=4)
    out_var = tk.StringVar(value=DEFAULT_OUT)
    ttk.Entry(row, textvariable=out_var, font=FONT_E).pack(side="left", fill="x", expand=True)
    ttk.Button(row, text="Browse", command=lambda: out_var.set(filedialog.askdirectory() or out_var.get())).pack(side="left", padx=6)

    ttk.Label(wrap, text="Bitrate:", font=FONT_L).pack(anchor="w", pady=(10,0))
    bit_var = tk.StringVar(value="320k"); ttk.Entry(wrap, textvariable=bit_var, width=8, font=FONT_E).pack(anchor="w")

    auth_var = tk.BooleanVar(); ttk.Checkbutton(wrap, text="Use my Spotify account (for private playlists)", variable=auth_var).pack(anchor="w", pady=(4,8))

    ttk.Label(wrap, text="Log:", font=FONT_L).pack(anchor="w")
    log_frame = ttk.Frame(wrap); log_frame.pack(fill="both", expand=True)
    log_txt = tk.Text(log_frame, wrap="word", font=("Consolas",11), bg="#202020", fg="#eaeaea", state="disabled")
    ys = ttk.Scrollbar(log_frame, command=log_txt.yview)
    log_txt.configure(yscrollcommand=ys.set); ys.pack(side="right", fill="y"); log_txt.pack(side="left", fill="both", expand=True)
    log_txt.bind("<Key>", lambda e: "break")

    q: queue.Queue[str] = queue.Queue()
    def log(msg: str): q.put(msg)
    def poll():
        try:
            while True:
                ln = q.get_nowait()
                log_txt.configure(state="normal"); log_txt.insert("end", ln+"\n"); log_txt.see("end"); log_txt.configure(state="disabled")
        except queue.Empty:
            pass
        root.after(120, poll)
    ...
    def _do_soundcloud(scurl: str, out_dir: str):
        ...
        if not queries:
            messagebox.showwarning("SoundCloud", "No tracks found in this SoundCloud set."); return
    ...
    def _stream(cmd: List[str], *, success: bool=False) -> bool:
        try:
            ...
            messagebox.showerror("SpotDL missing", "Run `pip install spotdl`."); return False
        for line in p.stdout: log(line.rstrip())
        p.wait(); return (p.returncode==0) if success else True

    def validate_and_go():
        url = url_var.get().strip(); svc = service_var.get()
        if svc == "Spotify" and "open.spotify.com/playlist/" not in url:
            messagebox.showerror("URL", "Enter a valid Spotify playlist URL."); return
        if svc == "SoundCloud" and "soundcloud.com/" not in url:
            messagebox.showerror("URL", "Enter a valid SoundCloud playlist URL."); return
        threading.Thread(target=run_spotdl, args=(url, out_var.get().strip() or DEFAULT_OUT, svc, auth_var.get()), daemon=True).start()

    ttk.Button(wrap, text="⬇ Download Playlist ⬇", command=validate_and_go, style="Accent.TButton").pack(fill="x", ipady=10, pady=(6,12))
    poll(); root.mainloop()
